...
void main(){
  ...
  col = mix(col, uColorC, fresnel * 0.45);
  col += fresnel * fresnel * 0.7;
...
/* ============================================================
   SCENE SETUP
   (no EffectComposer/UnrealBloomPass here — this build renders
   directly so it can run as a single offline HTML file with no
   ES modules; glow is compensated for in the shader/material
   intensities instead of a post-process bloom pass)
   ============================================================ */
...
function initScene() {
  ...
  pointWarm = new THREE.PointLight(0xff8a4c, 5, 20, 2);
  pointWarm.position.set(2, 1.5, -4);
  scene.add(pointWarm);
  pointCool = new THREE.PointLight(0x8b6bff, 4, 20, 2);
  pointCool.position.set(-3, -1, -8);
  scene.add(pointCool);
  const rim = new THREE.PointLight(0x4fe3d0, 3, 14, 2);
...
function buildGlassCube() {
  ...
  const finaleLight = new THREE.PointLight(0xffffff, 5, 10, 2);
  finaleLight.position.set(1.4, 1.1, -18);
  scene.add(finaleLight);
  const finaleLight2 = new THREE.PointLight(0x8b6bff, 4, 10, 2);
...
function updateSceneForProgress(p, dt, elapsed) {
  ...
  pointWarm.intensity = lerp(7, 12, Math.sin(elapsed * 0.4) * 0.5 + 0.5);
